...
public class CoreTest extends BaseTest {
    ...
    // НАДО ОБНОВИТЬ, ДОБАВИЛИСЬ НОВЫЕ ДАЙМОНДЫ
    ...
    // но захотелось написать именно так, так как это по сути один флоу - UI чек даймондов,
    // опять же, если повторяющиеся проверки вынести в хелпер или объединить, то тест будет
    // не такой нагруженный, но это уже задача на рефактор
    @Test
    ...
    // Тут, исходя из теста даймондов, решил все-же разграничить UI проверки от функциональных
    // поэтому этот тест на общие проверки наличия футора на странице
    // для проверки каждой ссылки в футоре необходимо писать отдельный тест, чем займусь позже, по необходимости
    @Test
